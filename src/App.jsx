...
import { 
  ...
  BarChart3, Settings, Search, Menu, ImageOff, X, Upload, ExternalLink, Paperclip, Loader2, FileCheck, Pencil, Save, Share2, Globe, Lock, Eye, Printer, FileDown, Sparkles, BrainCircuit, Rocket, ArrowLeft, Target, Award, Percent
} from 'lucide-react';
...
const COLLECTIONS = {
  ...
  GUESTS: 'guests',
  KPIS: 'kpis' // New Collection for KPIs
};
...
const generatePDF = (idea, analysisText = '') => {
  ...
  if (idea.rating) {
    evaluationHtml = `
      <div style="margin-top: 30px; margin-bottom: 30px; border: 1px solid #cbd5e1; border-radius: 4px; padding: 15px; background-color: #fff;">
        <h3 style="font-size: 16px; font-weight: bold; color: #1e293b; margin-top: 0; margin-bottom: 10px; border-bottom: 1px solid #e2e8f0; padding-bottom: 5px;">Manager Evaluation</h3>
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
           <span style="font-size: 18px; font-weight: bold; color: #0f172a;">Grade: ${idea.rating.grade}</span>
           <span style="font-size: 14px; color: #64748b;">Score: ${idea.rating.percentage}%</span>
        </div>
        <table style="width: 100%; border-collapse: collapse; font-size: 12px;">
          <tr style="background-color: #f1f5f9; text-align: left;">
            <th style="padding: 8px; border: 1px solid #e2e8f0;">KPI</th>
            <th style="padding: 8px; border: 1px solid #e2e8f0;">Weight</th>
            <th style="padding: 8px; border: 1px solid #e2e8f0;">Score (1-5)</th>
          </tr>
          ${idea.rating.details.map(kpi => `
            <tr>
              <td style="padding: 8px; border: 1px solid #e2e8f0;">${kpi.label}</td>
              <td style="padding: 8px; border: 1px solid #e2e8f0;">${kpi.weight}%</td>
              <td style="padding: 8px; border: 1px solid #e2e8f0;">${kpi.score}</td>
            </tr>
          `).join('')}
        </table>
      </div>
  ...
  const analysisHtml = analysisText ? `
    <div style="background-color: #f0fdf4; padding: 20px; border-radius: 4px; border: 1px solid #bbf7d0; margin-bottom: 30px;">
...
const Modal = ({ isOpen, onClose, title, children }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-sm animate-fade-in overflow-y-auto">
      <div className="bg-white rounded-sm shadow-2xl w-full max-w-4xl min-h-[50vh] flex flex-col animate-scale-up my-8">
        <div className="flex justify-between items-center p-6 border-b border-slate-100 bg-slate-50 flex-none">
...
const RatingSystem = ({ idea, onRate, kpis }) => {
  ...
  useEffect(() => {
    // Initialize scores if idea already has rating
  ...
  const calculateGrade = () => {
    ...
    kpis.forEach(kpi => {
      ...
      // Formula: (Score / 5) * Weight
      const weightedContribution = (score / 5) * kpi.weight;
      totalWeightedScore += weightedContribution;
      totalWeight += parseInt(kpi.weight);
      details.push({ label: kpi.label, weight: kpi.weight, score });
    });

    // Normalize if weights don't equal 100 (just in case, though Admin should ensure)
    // Actually, prompt says: Total Score = Sum(KPI Score * Weight).
    // Let's assume weights sum to 100.
    // If score is 5 for all, and weights sum to 100.
    // (5/5)*30 + (5/5)*20 ... = 30 + 20 ... = 100.
    // So totalWeightedScore is essentially the percentage.
  ...
  return (
    <div className="bg-slate-50 border border-slate-200 rounded-sm p-6 mb-6">
      <h4 className="text-sm font-bold text-slate-900 uppercase tracking-widest mb-4 flex items-center gap-2">
        <Target className="w-4 h-4" /> Manager Evaluation
      </h4>
      <div className="space-y-4">
        {kpis.map((kpi, idx) => (
          <div key={idx} className="flex flex-col sm:flex-row sm:items-center gap-4">
            <div className="flex-1">
              <div className="text-sm font-bold text-slate-700">{kpi.label} <span className="text-slate-400 text-xs font-normal">({kpi.weight}%)</span></div>
            ...
            <div className="flex items-center gap-3">
              ...
              />
              <span className="w-8 text-center font-bold text-slate-900 border border-slate-200 bg-white rounded-sm py-1">{scores[kpi.label] || 0}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="mt-6 pt-4 border-t border-slate-200 flex justify-between items-center">
        <div>
           <div className="text-xs text-slate-500 uppercase tracking-widest">Calculated Grade</div>
           <div className="flex items-baseline gap-2">
             <span className="text-3xl font-extrabold text-indigo-900">{currentResult.grade}</span>
             <span className="text-lg font-medium text-slate-600">({currentResult.percentage}%)</span>
           </div>
        </div>
        <Button onClick={submitRating} className="px-6">Save Rating</Button>
      </div>
    </div>
  );
};
    ...
    <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={idea.formTitle}>
        ...
        {isEmployeeView && idea.rating && (
           <div className="bg-emerald-50 border border-emerald-100 p-4 rounded-sm mb-6">
             ...
             <div className="flex items-center gap-4">
                <div className="text-3xl font-extrabold text-emerald-900">{idea.rating.grade}</div>
                <div className="text-sm text-emerald-700">{idea.rating.percentage}% Score</div>
             </div>
           </div>
        )}
      ...
      <div className="bg-slate-50 border border-slate-200 p-4 rounded-sm mb-6">
        <div className="flex gap-2 items-end mb-4">
          <div className="flex-1">
             ...
             <input className="w-full px-3 py-2 border border-slate-300 rounded-sm text-sm" value={newKPI.label} onChange={e => setNewKPI({...newKPI, label: e.target.value})} />
          </div>
          <div className="flex-1">
             <label className="block text-[10px] uppercase font-bold text-slate-400 mb-1">Description</label>
             <input className="w-full px-3 py-2 border border-slate-300 rounded-sm text-sm" value={newKPI.description} onChange={e => setNewKPI({...newKPI, description: e.target.value})} />
          </div>
          <div className="w-24">
             <label className="block text-[10px] uppercase font-bold text-slate-400 mb-1">Weight %</label>
             <input type="number" className="w-full px-3 py-2 border border-slate-300 rounded-sm text-sm" value={newKPI.weight} onChange={e => setNewKPI({...newKPI, weight: e.target.value})} />
          </div>
          <Button onClick={add}>Add</Button>
        </div>
        <div className="flex justify-between items-center text-xs font-bold uppercase tracking-widest text-slate-500">
           <span>Total Weight: {totalWeight}%</span>
           {totalWeight !== 100 && <span className="text-amber-600">Warning: Total should be 100%</span>}
        </div>
      </div>

      <div className="space-y-2">
        {kpis.map((k, i) => (
          <div key={i} className="flex justify-between items-center p-3 border rounded-sm hover:bg-slate-50">
            <div>
              <div className="font-bold text-slate-800 text-sm">{k.label} <span className="text-slate-500">({k.weight}%)</span></div>
              <div className="text-xs text-slate-400">{k.description}</div>
            </div>
            <button onClick={() => remove(i)} className="text-slate-400 hover:text-red-600 p-2"><Trash2 className="w-4 h-4" /></button>
          </div>
        ))}
      </div>
    </Card>
...
// ... [Existing UserApprovalRow, UserManagement, GuestManagement, DepartmentManager, FormBuilder components remain the same, just included below]
...
const AdminPortal = ({ showToast }) => {
  ...
  useEffect(() => {
    ...
    const unsub5 = onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', COLLECTIONS.KPIS), async (s) => {
       if (s.empty) {
         ...
         // We will store all KPIs in a SINGLE document for easy array management in this simple app
  ...
  // ... (Existing handlers: approveUser, updateUser, deleteUser, etc. remain the same)
...
// ... [Existing EmployeePortal, ManagerPortal, GuestView, etc. need to accept kpis prop]
...
const ManagerPortal = ({ currentUser, showToast }) => {
  ...
  const handleComment = useCallback(async (id, text) => {
    await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', COLLECTIONS.IDEAS, id), {
      comments: arrayUnion({ id: Date.now(), author: currentUser.name, text, date: new Date().toISOString() })
    });
  ...
  const handleTogglePublic = useCallback(async (id, isPublic) => {
    await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', COLLECTIONS.IDEAS, id), { isPublic: isPublic });
          ...
          <>
            ...
            <div className="grid grid-cols-1 gap-6 py-2">
                {Object.entries(selectedIdea.formData).map(([k, v]) => {
                  ...
                  const isImage = isUrl && (v.includes('drive.google.com') && !v.includes('view?usp=drivesdk') || v.match(/\.(jpeg|jpg|gif|png)$/i) || v.includes('googleusercontent'));
                  return (
                  <div key={k} className="group">
...
// ... [Include Main App, Login, Register, UserManagement components - ensure standard exports]
